...
void fs_init(void)
{
    FS_OPERATE_FUNCTIONS_SET(g_operate_functions[FS_TYPE_YIREN], yiren);
...
int fs_cp(const char* src_path, const char* new_path) {
    ...
    if (ctrl->opfuncs->cp(ctrl->device, ctrl->fs_ctrl, path_remain(src_abspath), path_remain(new_abspath))) {
        return FS_SUCCESS;
    } else {
        return FS_ERROR;
    }
