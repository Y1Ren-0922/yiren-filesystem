...
// 同理，一个简单的实现，没考虑内存布局和字节序
size_t inode_bin_size()
{
...
bool inode_load(dev_inode_ctrl_t* dev_inode_ctrl, inode_no_t no, inode_t* inode)
{
    ...
    bool success = block_read(dev_inode_ctrl->device, dev_inode_ctrl->block_size / BYTES_PER_SECTOR, block_no, buf);
    if (!success)
        return false;
...
bool inode_dump(dev_inode_ctrl_t* dev_inode_ctrl, inode_no_t no, inode_t* inode)
{
    ...
    *((inode_t*)(buf + offset)) = *inode;
    ...
    success = block_write(dev_inode_ctrl->device, dev_inode_ctrl->block_size / BYTES_PER_SECTOR, block_no, buf);
    if (!success) {
