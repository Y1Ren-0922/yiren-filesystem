...
// 打开底层文件，注意：superblock的内存使用由用户决定
...
// 时间属性
timestamp_t base_file_accessed_file(const base_file_t* base_file);
...
bool base_file_truncate(base_file_t* base_file, fsize_t size); // 截断文件内容
...
bool base_file_ref(device_handle_t device, superblock_t* sb, inode_no_t inode_no); // 文件引用加1
bool base_file_unref(device_handle_t device, superblock_t* sb, inode_no_t inode_no); // 文件引用减1， 到0就删除