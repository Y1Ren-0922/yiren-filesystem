...
static bool dir_add(device_handle_t device, yiren_filesystem_t* fs, inode_no_t dir, const char* name, inode_no_t no);
static bool dir_del(device_handle_t device, yiren_filesystem_t* fs, inode_no_t dir, const char* name);
static bool dir_tree_locate(device_handle_t device, yiren_filesystem_t* fs, inode_no_t start, const char* relative_path, bool* p_exist, inode_no_t* p_no);
static bool dir_roottree_locate(device_handle_t device, yiren_filesystem_t* fs, const char* path, bool* p_exist, inode_no_t* p_no);
...
static bool yiren_file_init(yiren_file_t* file, device_handle_t device, yiren_filesystem_t* fs, const char* path);

yiren_file_t* yiren_open(device_handle_t device, yiren_filesystem_t* fs, const char* path)
{
    yiren_file_t* file = FT_NEW(yiren_file_t, 1);
    if (!yiren_file_init(file, device, fs, path)) {
        return NULL;
    }
    return file;
}
...
{
    struct fs_stat st;
    if (!yiren_stat(device, fs, path, &st)) {
        return false;
    }
    ...
    yiren_file_t* file = yiren_open(device, fs, path);
    if (file == NULL) {
        return false;
    }
    ...
    int count = yiren_read(file, buf, read_size);
    if (count != read_size) {
        return false;
    }
    buf[size] = '\0';
...
{
    yiren_dir_t* dir = FT_NEW(yiren_dir_t, 1);
